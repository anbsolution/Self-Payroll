...
interface AttendanceDao {
    ...
    @Query("SELECT * FROM attendance WHERE date = :date LIMIT 1") fun observeByDate(date: String): Flow<AttendanceEntity?>
    ...
    @Query("SELECT * FROM attendance WHERE syncState = 'PENDING' OR syncState = 'FAILED'") suspend fun pending(): List<AttendanceEntity>
